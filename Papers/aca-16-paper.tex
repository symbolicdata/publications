...
Representation of metadata in RDF format together with the SPARQL querying
language are well suited to explore data and nowadays even emerged to a certain
standard within the Linked Data world. Let's demonstrate the search and filter
facility on the metadata of two different collections by two
examples\footnote{See also \url{http://wiki.symbolicdata.org/MoreQueries}.}.
We assume the reader to be familiar with RDF and SPARQL basics. 
...
\end{quote}
wee took from the Normaliz collection provided in 2015 by Tim R\"omer (entry~4
...
rdfs:label {\dq}TestSet/graph10\_001\_k5free{\dq} ;\\
rdfs:comment {\dq}{\dq}{\dq}Verification of a conjecture of Sturmfels and 
\\\>Sullivant on normality of cut polytopes for some specific
\\\> graphs [SS], [BHIKS], [O], [S].{\dq}{\dq}{\dq} ;\\
sd:hasOrigin {\dq}{\dq}{\dq}Normaliz Collection, Tim Roemer 2015-03.
\\\>From Example 8 of the Normaliz collection{\dq}{\dq}{\dq} ;\\
sd:hasNumberOfExtremeRays {\dq}512{\dq}{\xsdprefix}xsd:integer ;\\
sd:hasSupportingHyperplanes {\dq}352{\dq}{\xsdprefix}xsd:integer ;\\
sd:numberOfRows {\dq}512{\dq}{\xsdprefix}xsd:integer ;\\
sd:numberOfColumns {\dq}48{\dq}{\xsdprefix}xsd:integer ;\\
sd:hasNumberOfHilbertBasisElements {\dq}512{\dq}{\xsdprefix}xsd:integer ;\\
sd:hasRank {\dq}25{\dq}{\xsdprefix}xsd:integer ;\\
...
the last line gives the URL to a zip-file with the files
\texttt{graph10\_001\_k5free.in} (input in Normaliz format) and
